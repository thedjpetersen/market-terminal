...
use crate::features::chat::{
    ...
    ChatMessageKind, validate_chat_message,
};
    ...
    pub fn new() -> Self {
        ...
        let messages = [
            (
                "08:31",
                "rates",
                "US 10Y +4.2bp; curve bear-steepening into the open.",
            ),
            (
                "08:33",
                "equity-desk",
                "Semis leading premarket. Watching breadth after cash open.",
            ),
            ("08:35", "fx", "DXY 103.42; EURUSD holding 1.0840 support."),
            (
                "08:38",
                "macro",
                "Consensus still sees the next cut in September.",
            ),
            (
                "08:41",
                "market-bot",
                "ALERT · NVDA crossed 125.00 on above-average volume.",
            ),
        ];
        let mut events = vec![
...
impl ChatGateway for DemoChatGateway {
    ...
    fn send_message(&self, message: &str) -> Result<(), ChatGatewayError> {
        ...
        *sequence += 1;
        self.events
            .lock()
            .expect("demo chat events lock")
            .push(ChatEvent::Message(ChatMessage {
                sequence: *sequence,
                time: "NOW".to_owned(),
    ...
    fn reconnect(&self) -> Result<(), ChatGatewayError> {
        self.events.lock().expect("demo chat events lock").extend([
            ChatEvent::State(ChatConnectionState::Reconnecting),
            ChatEvent::State(ChatConnectionState::Connected),
        ]);
        Ok(())
    }
