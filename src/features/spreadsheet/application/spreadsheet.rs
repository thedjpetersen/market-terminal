...
impl Spreadsheet {
    ...
    pub fn from_workbook(workbook: Workbook) -> Self {
        Self { workbook, undo_stack: Vec::new(), redo_stack: Vec::new() }
    }
    ...
    pub fn add_sheet(&mut self, name: impl Into<String>) -> Result<usize, SpreadsheetError> {
        ...
        self.record_change(|workbook| {
            workbook.add_sheet(name).map_err(SpreadsheetError::Workbook)
        })
    }
    ...
    pub fn rename_active_sheet(&mut self, name: impl Into<String>) -> Result<(), SpreadsheetError> {
        let name = name.into();
        self.record_change(|workbook| {
            workbook.rename_active_sheet(name).map_err(SpreadsheetError::Workbook)
        })
    }

    pub fn remove_active_sheet(&mut self) -> Result<(), SpreadsheetError> {
        self.record_change(|workbook| {
            workbook.remove_active_sheet().map(|_| ()).map_err(SpreadsheetError::Workbook)
        })
    }
    ...
    pub fn export_csv(&self) -> String {
        ...
        let Some(max_column) = sheet.populated_cells().map(|(address, _)| address.column()).max() else {
            return String::new();
        };
        let max_row = sheet
            .populated_cells()
        ...
        for row in 1..=max_row {
            ...
            for column in 1..=max_column {
                ...
                let raw = sheet.cell(address).map(|cell| cell.raw()).unwrap_or_default();
...
impl fmt::Display for CsvError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TooManyRows(rows) => write!(formatter, "CSV has {rows} rows; the limit is {MAX_ROWS}"),
            Self::TooManyColumns { row, columns } => {
                write!(formatter, "CSV row {row} has {columns} columns; the limit is {MAX_COLUMNS}")
            }
        }
    }
}
...
fn write_csv_field(output: &mut String, field: &str) {
    if field.chars().any(|character| matches!(character, ',' | '"' | '\r' | '\n')) {
        output.push('"');
...
mod tests {
    ...
    fn undo_and_redo_restore_formula_dependencies_and_branch_history() {
        ...
        assert_eq!(spreadsheet.cell("B1").unwrap().value, CellValue::Number(50.0));
        assert!(spreadsheet.can_undo());
        assert!(spreadsheet.undo());
        assert_eq!(spreadsheet.cell("A1").unwrap().raw, "10");
        assert_eq!(spreadsheet.cell("B1").unwrap().value, CellValue::Number(20.0));
    ...
    fn bulk_edits_validate_first_and_undo_as_one_operation() {
        ...
        spreadsheet.set_cells([("A1", "1"), ("A2", "2"), ("A3", "3")]).unwrap();
        assert_eq!(spreadsheet.cell("A3").unwrap().value, CellValue::Number(3.0));
        assert!(spreadsheet.undo());
        assert_eq!(spreadsheet.cell("A1").unwrap().value, CellValue::Blank);
